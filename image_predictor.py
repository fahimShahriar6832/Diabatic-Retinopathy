...
with st.sidebar:
    ...
    selected = option_menu('Diabatic-Retinopathy',
                          ...
                          default_index=0)
st.title('Diabetes Retinopathy using ML :drop_of_blood:')  
...
if (selected == 'Fundas Image'):
    ...
    if uploaded_file is not None:
        ...
        if result[0][0] > result[0][1]:
          st.write("Diabetic Retinopathy :cry: [{:.2f}% accuracy]".format((result[0][0]*100)))
        else:
          st.write("NO Diabetic Retinopathy :grinning: [{:.2f}% accuracy]".format((result[0][1])*100))
